...
public class CustomDatePicker {
    ...
    public CustomDatePicker(Context context, ResultHandler resultHandler, String startDate, String endDate) {
        if (DateUtil.isValidDate("yyyy-MM-dd HH:mm", startDate, endDate)) {
            canAccess = true;
            ...
            startCalendar.setTime(DateUtil.convert(startDate));
            endCalendar.setTime(DateUtil.convert(endDate));
            initDialog();
            initView();
        }
    }
    ...
    public void show() {
        if (canAccess) {
            if (startCalendar.getTime().getTime() < endCalendar.getTime().getTime()) {
                ...
                initTimer();
                addListener();
                setSelectedTime(DateUtil.dateFormat(new Date()));
